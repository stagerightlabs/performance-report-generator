...
class ReportGenerator
{
    ...
    /**
     * Retrieve a single sentence from the sentences array
     * @param  string $type      The sentence category to draw upon
     * @param  array  $excluding Sentence indices to be skipped
     * @return object|null
     */
    protected function fetchSentence($type, array $excluding = array())
    {
        // Filter sentences by type and exclude already used ones
        $available = [];
        foreach ($this->sentences as $index => $sentence) {
            if ($sentence['type'] === $type && !in_array($index, $excluding)) {
                $available[$index] = $sentence;
            }
        }
        ...
        $sentence = $available[$randomIndex];

        // Return as an object to match the original DB result format
        return (object) [
            'id' => $randomIndex,
            'text' => $sentence['text'],
            'type' => $sentence['type']
